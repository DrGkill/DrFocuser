...
//
// ASCOM Focuser driver for DrFocuser
//
...
namespace ASCOM.DrFocuser
{
    //
    // Your driver's DeviceID is ASCOM.DrFocuser.Focuser
    //
    // The Guid attribute sets the CLSID for ASCOM.DrFocuser.Focuser
    // The ClassInterface/None addribute prevents an empty interface called
    // _DrFocuser from being created and used as the [default] interface
    //
    ...
    /// ASCOM Focuser Driver for DrFocuser.
    /// </summary>
    [Guid("5679ed87-d804-4e80-a735-e8a403b718a8")]
    ...
    public class Focuser : IFocuserV2
    {
        ...
        /// </summary>
        internal static string driverID = "ASCOM.DrFocuser.Focuser";
        ...
        internal static string comPortDefault = "COM8";
        ...
        public void Halt()
        {
            tl.LogMessage("Halt", "Stopping Motor !");
        ...
        public void Move(int Position)
        {
            ...
            SerialConnection.Transmit(Position.ToString() + "#");
            /*
            int MoveNsteps = Math.Abs(focuserPosition - Position);
            // If the asked position is below focuser position, set clockwise
            if (Position < focuserPosition)
            {
                SerialConnection.Transmit("C#");
                SerialConnection.Transmit(MoveNsteps.ToString() + "#");
                
            }
            else
            {
                // Else we want to move anticlockwise 
                SerialConnection.Transmit("A#");
                SerialConnection.Transmit(MoveNsteps.ToString() + "#");
            }*/
        ...
        public int Position
        {
            get
            {
                ...
                tl.LogMessage("PositionTEST", "\"" + received.ToString() + "\"");
                try
                {
                    focuserPosition = Int32.Parse(received.Trim());
                }
                catch (FormatException ex )
                {
                    throw new Exception($"TEST : ->{received}<-", ex);
                }
